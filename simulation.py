...
class Simulation(object):
    def __init__(self, configuration, transactions):
        ...
        for i, steps in enumerate(transactions):
            ...
            shard.process_tx(tx, shard.blocks[-1].outbox, shard.blocks[-1].pending_txs, shard.blocks[-1].locks)
    ...
    def step(self):
        ...
        for shard in self.shards:
            ...
            for tx in shard.blocks[-1].pending_txs:
                shard.process_tx(tx, outbox, pending_txs, locks)
            ...
            for neighbor_id in [shard.parent_id] + shard.child_ids:
                ...
                for msg in self.shards[neighbor_id].blocks[-1].outbox[shard.shard_id]:
                    shard.process_message(msg, outbox, pending_txs, locks)
                    ...
                    if msg.msg_type == Message.ROLLBACK:
                        self.txstates[msg.tx.tx_id].latest_rollback = max(self.txstates[msg.tx.tx_id].latest_rollback, msg.tx.step_id)
                        if self.txstates[msg.tx.tx_id].latest_rollback == self.txstates[msg.tx.tx_id].latest_step:
            ...
            new_blocks[shard.shard_id] = Block(shard.blocks[-1], outbox, locks, pending_txs)
...
if __name__ == "__main__":
    ...
    for i in range(MAX_STEPS):
        ...
        for txstate in simulation.txstates:
            print("TX %s: step %s, rollback %s, return %s" % (txstate.tx.tx_id, txstate.latest_step, txstate.latest_rollback, txstate.latest_return))
